...
namespace SSY.Blazor.Pages.CollectionAndProduct.Components.Collections
{
    public partial class Detail
    {
        ...
        public string Module = "Collection Detail";
        ...
        public async Task OnSubmitHandler()
        {
            try
            {
                ...
                bool isSuccessCreateReservation = false;
                bool isNoChanges = false;

                List<CreateRollReservationModel> rollReservations = new();

                int currentFabricCount = 0;
                int newFabricCount = 0;

                CollectionDto.ColorOptions.ForEach(x => {
                    currentFabricCount += x.Fabrics.Count();
                });

                CollectionModel.ColorSelectionList.ForEach(x => {
                    newFabricCount += x.FabricSelectionList.Count();
                });

                //Console.WriteLine("currentFabricCount : " + currentFabricCount);
                //Console.WriteLine("newFabricCount : " + newFabricCount);

                List<Guid> rollIdToReserve = new();
                List<Guid> materialIdToReserve = new();
                List<Guid> materialIdToDeleteReserve = new();
                List<Guid> rollIdToDeleteReserve = new();

                if (newFabricCount > currentFabricCount)
                {
                    Console.WriteLine("New Fabrics Added");

                    CollectionModel.ColorSelectionList.ForEach(colorOption => {

                        colorOption.FabricSelectionList.ForEach(fabric => {

                            if (!CollectionDto.ColorOptions.Any(x => x.Fabrics.Any(f => f.MaterialId == fabric.MaterialId)))
                            {
                                if (!materialIdToReserve.Contains(fabric.MaterialId))
                                    materialIdToReserve.Add(fabric.MaterialId);

                                var material = MaterialListModel.Result.Items.Find(x => x.Id == fabric.MaterialId);

                                material.RollsAndLocations.ForEach(roll => {

                                    fabric.SelectedRollIds.ForEach(rollId => {

                                        var reservation = roll.RollReservations.Find(x => x.CollectionId == colorOption.CollectionId && x.RollId == rollId);

                                        if (reservation == null)
                                        {
                                            if (!rollIdToReserve.Contains(rollId))
                                                rollIdToReserve.Add(rollId);
                                        }
                                    });
                                });
                            }
                ...
                else if (newFabricCount < currentFabricCount)
                {
                    Console.WriteLine("Some Fabrics Deleted");
                    ...
                    CollectionDto.ColorOptions.ForEach(colorOption => {
                        colorOption.Fabrics.ForEach(fabric => {
                            ...
                            CollectionModel.ColorSelectionList.ForEach(x => {
                                var selectedFabric = x.FabricSelectionList.Find(f => f.MaterialId == material.Id);

                                if (selectedFabric == null)
                                {
                                    if (!materialIdToDeleteReserve.Contains(material.Id))
                                        materialIdToDeleteReserve.Add(material.Id);
                                }
                ...
                else
                {
                    ...
                    CollectionModel.CurrentFabricCount = currentFabricCount;
                ...
                if (materialIdToReserve.Any())
                {
                    materialIdToReserve.ForEach(materialId =>
                    {
                        var material = MaterialListModel.Result.Items.Find(m => m.Id == materialId);

                        List<RollReservation> RollReservations = new();

                        if (rollIdToReserve.Any())
                        {
                            rollIdToReserve.ForEach(rollId =>
                            {
                        ...
                        CreateRollReservationModel rollReservation = new();

                        if (RollReservations.Any())
                        {
                            List<string> influencers = new();

                            CollectionModel.influencers.ForEach(x => influencers.Add(x.InfluencerFullName));

                            rollReservation.CollectionId = CollectionDto.Id.ToString();
                            rollReservation.IsActive = CollectionDto.IsActive;
                            rollReservation.MaterialId = materialId;
                            rollReservation.RollReservations = RollReservations;
                            rollReservation.CollectionName = CollectionModel.Name;
                            rollReservation.InfluencerNames = string.Join(", ", influencers);

                            rollReservations.Add(rollReservation);
                        }
                    });
                }
                ...
                if (materialIdToDeleteReserve.Any())
                {
                    materialIdToDeleteReserve.ForEach(materialId => {
                        ...
                        if (material != null)
                        {
                            CollectionDto.ColorOptions.ForEach(colorOption => {
                                ...
                                MaterialRoll materialRoll = new();
                                List<Guid> rolls = new();

                                var selectedFabric = colorOption.Fabrics.Find(x => x.MaterialId == materialId);

                                if (selectedFabric != null)
                                {
                                    selectedFabric.Rolls.ForEach(roll => {
                                        if (!rolls.Contains(roll.RollId))
                                            rolls.Add(roll.RollId);
                                    });

                                    materialRoll.MaterialId = materialId;
                                    materialRoll.Rolls = rolls;

                                    materialRollNeedToDeleteReservation.Add(materialRoll);
                                }
                ...
                if (isNoChanges == false)
                {
                    if (rollReservations.Count == 0)
                        isSuccessCreateReservation = true;
                    else
                        isSuccessCreateReservation = await _materialService.CreateRollReservation(rollReservations);
