...
namespace SSY.Blazor.Pages.Products.Components.ProductMediaFile
{
    public partial class ProductMediaFile
    {
        ...
        protected override async Task OnParametersSetAsync()
        {
            ...
            if(result == "1stFitSampleImages(OEM)")
            {
                titleModal = "firstfitSample";
            }
            else if(result == "2ndFitSampleImages(OEM)")
            {
                 titleModal = "secondfitSample";
            }
            else if(result == "1stFitSampleImages(Influencer)")
            {
                   titleModal ="firstfitSampleInfluencer";
            }
            else if(result == "2ndFitSampleImages(Influencer)")
            {
                 titleModal = "secondfitSampleInfluencer";
            }
            else
            {
                titleModal = result;
        ...
        public async void ZoomImage(string image, string title)
        {
            if(title == "Design Draft")
            {
                ModalImage = image;
                Console.WriteLine(JsonSerializer.Serialize(ModalImage));
