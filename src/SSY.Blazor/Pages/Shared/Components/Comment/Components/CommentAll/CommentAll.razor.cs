...
namespace SSY.Blazor.Pages.Shared.Components.Comment.Components.CommentAll
{

    public partial class CommentAll
    {
        #region Injections
        [Inject]
            ...
            foreach (var item in GetAllCommentOutputModel?.Result?.Items)
            {
                if(item.File != null)
                {
                    ...
                    UploadedFiles.ForEach(x =>
                    {
                        ...
                        item.Files.Add(new SSY.Blazor.HttpClients.Data.Inventory.TimelineAndComments.Model.Comments.Files() { Value = x,Extension = file.Extension.ToString() });
                    });
                }
            }
        }
