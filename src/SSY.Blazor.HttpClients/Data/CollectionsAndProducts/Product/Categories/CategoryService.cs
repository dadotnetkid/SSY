...
using SSY.Blazor.HttpClients.Data.CollectionsAndProducts.Product.Categories.Model;

namespace SSY.Blazor.HttpClients.Data.CollectionsAndProducts.Product.Categories
{
    public class ProductCategoryService
    {
        public ProductCategoryService(IJSRuntime js,
        ...
        public async Task<GetAllProductCategoryOutputModel> GetAllProductCategory(string? keyword, string? sorting, int? skipCount, int? maxResultCount)
        {
            var url = $"{_configuration["App:ProductServerRootAddress"]}/category?";
            ...
            GetAllProductCategoryOutputModel result = new();

            if (response.IsSuccessStatusCode)
            {
                using var responseStream = await response.Content.ReadAsStreamAsync();

                var data = await JsonSerializer.DeserializeAsync<GetAllProductCategoryOutputModel>(responseStream);
                result = data;
            }
            else
            {
        ...
        public async Task CreateProductCategory(CreateProductCategoryModel input)
        {
        ...
        {
            var url = $"{_configuration["App:ProductServerRootAddress"]}/category/{id}";
            ...
            ProductCategoryModel result = new();

            if (response.IsSuccessStatusCode)
            {
                using var responseStream = await response.Content.ReadAsStreamAsync();

                var data = await JsonSerializer.DeserializeAsync<ProductCategoryModel>(responseStream);
                result = data;
            }
            else
            {
        ...
        public async Task UpdateProductCategory(UpdateProductCategoryModel input)
        {
            var json = JsonSerializer.Serialize(input);
            Console.WriteLine(json);

            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var url = $"{_configuration["App:ProductServerRootAddress"]}/category/{input.Id}";
        ...
        {
            DeleteProductCategoryModel input = new() { Id = id };

            var json = JsonSerializer.Serialize(input);
            Console.WriteLine(json);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var url = $"{_configuration["App:ServerRootAddress"]}/category/{id}";
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                await GetAllProductCategory(null, null, null, 100);
